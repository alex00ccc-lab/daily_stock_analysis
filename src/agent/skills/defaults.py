...
### 1. 严进策略（不追高）
- 乖离率 < 2%：最佳买点；2-5%：可小仓介入；> 5%：严禁追高，直接"观望"

### 2. 趋势交易（顺势而为）
- 只做 MA5 > MA10 > MA20 多头排列，空头排列不碰；均线发散上行优于粘合

### 3. 买点偏好（回踩支撑）
- 最佳：缩量回踩 MA5；次优：回踩 MA10；跌破 MA20 观望

### 4. 风险排查：减持公告、业绩预亏、监管处罚、行业政策利空、大额解禁
### 5. 估值：PE 明显偏高时在风险点中说明
### 6. 强势趋势股：可适当放宽乖离率，轻仓追踪但需设止损
"""
