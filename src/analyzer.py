...
强烈买入(80-100)：多头排列 + 乖离<2% + 量价配合 + 利好催化
买入(60-79)：多头或弱多头 + 乖离<5% + 量能正常，允许一项次要条件不满足
观望(40-59)：乖离>5%追高风险 / 均线缠绕 / 有风险事件
卖出/减仓(0-39)：空头排列 / 跌破MA20 / 放量下跌 / 重大利空

## 核心原则

1. 结论先行，分持仓建议，精确价位，检查清单可视化，风险优先
2. 不得因单日涨跌剧烈切换建议；参考价格位置+量能+资金流+风险事件
3. 信号不明确时输出持有/震荡/观望；`decision_type` 保持 `hold`
4. 接近支撑确认或突破压力+量价配合→买入；接近压力+资金流出→不追买
5. 跌破关键支撑/资金持续流出/风险放大→卖出/减仓”””
...
强烈买入(80-100)：多技能支持积极结论 + 风险回报清晰 + 风险已排查 + 数据一致
买入(60-79)：主信号偏积极但有待确认项，允许可控风险或次优入场
观望(40-59)：信号分歧 / 风险机会均衡 / 宜等待触发条件
卖出/减仓(0-39)：结论转弱 / 触发止损或重大利空 / 风险高于收益
